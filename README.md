...
- 仓库中的 [sources.yaml](sources.yaml) 与 `repo.csv` 仅收录第三方 POC 模板仓库的**链接清单**，供 `gobas sync` 一键拉取使用，**本项目不包含任何第三方模板的实体内容**
- 清单中的仓库均属其各自作者所有，版权归原作者/原仓库所有，列表整理仅为学习与研究之便利，**仅供学习参考，请勿用于未授权环境**
- 各源仓库许可协议不一（部分无 LICENSE，默认保留所有权利），同步后请自行遵守对应仓库的开源条款
- 如清单中某仓库存在侵权、违规或权利人要求移除，请通过 GitHub Issue 反馈，核实后将在下个版本移除对应链接
